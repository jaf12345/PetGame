...
public class Cat extends VirtualPet {
    ...
    public Cat(String name, int hungerLevel, int energyLevel, int happinessLevel) {
        setName(name);
    ...
    public void eat() {
        if (getHungerLevel()>=10) {
            ...
            System.out.println(getName()+" has finshed grazing there food\n");
        }
        if(getHungerLevel()==0) {
    ...
    void scratch() {
        System.out.println(getName()+" is using there scracth board!\n");
        if (getHappinessLevel()<=90) {
            setHappinessLevel(getHappinessLevel()+HAPPINESS_INCREASE);
        }
    }
